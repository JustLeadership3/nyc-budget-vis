...
function processCapitalExpenditures(json) {
  ...
  while (i < json.length){
    ...
    if ("agency_capital_expenditures_by_purpose" in possibleDepartment) {
      DepartmentName = possibleDepartment["agency_capital_expenditures_by_purpose"]
      if (DepartmentName.includes("TOTAL"))
      bulkTotals.push(possibleDepartment)
    }  
  }
  ...
  while (k < bulkTotals.length) {
    ...
    let departmentHash = {};
    const agencyDigestedName = oneDepartment["agency_capital_expenditures_by_purpose"].substring(6).toUpperCase();
